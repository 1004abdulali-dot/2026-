...
export default function App() {
  const [role, setRole] = useState<Role>('teacher'); const [name, setName] = useState(''); const [joined, setJoined] = useState(false); const [state, setState] = useState<State>(empty); const [loading, setLoading] = useState(true); const [error, setError] = useState('')
  ...
  const title = ['재료 탐정이 되어 봐요!', '궁금한 점을 모아 봐요!', '재료의 느낌을 모아 봐요!'][state.step - 1]
  ...
  return <div className="app-shell"><header className="topbar"><div className="brand"><span>🎨</span><b>톡톡! 재료 탐험</b></div><div className="live"><i /> 실시간 함께 수업 중</div><div className="user-badge">{role === 'teacher' ? '👩‍🏫 선생님' : `🧒 ${name}`}</div></header><main className={role === 'teacher' ? 'classroom teacher-layout' : 'classroom'}><section className="lesson-area"><div className="lesson-header"><span className="step-chip">{state.step}단계 / 3단계</span><h1>{title}</h1><p>{state.step === 1 ? '그림을 자세히 관찰하고 재료를 맞혀 보세요.' : state.step === 2 ? '재료를 골라 질문을 보내면 친구들 화면에도 나타나요.' : '같은 단어가 많아질수록 더 크게 자라나요!'}</p></div>{loading ? <div className="status">수업 내용을 불러오고 있어요…</div> : <>{state.step === 1 ? <StepOne role={role} studentName={name} votes={state.votes} drawings={state.drawings} onVote={(imageId, material) => post('votes', { student_name: name, image_id: imageId, material })} onImageUpload={(imageId, imageData) => post('drawing-image', { image_id: imageId, image_data: imageData })} onAnswerChange={(imageId, material) => post('drawing-answer', { image_id: imageId, answer_material: material })} onRevealAllAnswers={() => post('drawing-answers/reveal-all')} /> : <MaterialGrid mode={state.step === 2 ? 'question' : 'keyword'} messages={state.messages} />}{role === 'student' && state.step > 1 && <StudentInput step={state.step} onSend={(material, text) => post(state.step === 2 ? 'questions' : 'keywords', { student_name: name, material, text })} />}</>}{error && <p className="connection-error">{error}</p>}</section>{role === 'teacher' && <TeacherPanel step={state.step} votes={state.votes} messages={state.messages} onStep={(step) => post(`step/${step}`)} onReset={resetSession} />}</main>{role === 'student' && <footer>선생님이 다음 단계로 이동하면 화면이 자동으로 바뀌어요. 🌟</footer>}</div>
}
